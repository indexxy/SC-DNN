...
def sum_sc(x: np.ndarray, conc=1):
...
def dot_sc(x: np.ndarray, y: np.ndarray, conc=1):
    ...
    result = np.empty((x.shape[0], y.shape[1], x.shape[2] * conc), dtype=np.bool)
    y_t = y.transpose(1, 0, 2)
    for i in range(x.shape[0]):
        for j in range(y.shape[1]):
            result[i][j] = sum_sc(mul_sc(x[i], y_t[j]), conc=conc)
